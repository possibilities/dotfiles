...
if ! sudo systemctl is-active --quiet dnsmasq; then
    echo "dnsmasq failed to start; restoring previous configuration" >&2
    [ -f /etc/dnsmasq.conf.backup ] && sudo mv /etc/dnsmasq.conf.backup /etc/dnsmasq.conf
    [ -f /etc/resolv.conf.backup ] && sudo mv /etc/resolv.conf.backup /etc/resolv.conf
    exit 1
fi
...
if [ -L /etc/resolv.conf ]; then
    echo "Warning: /etc/resolv.conf is a symlink and likely managed by another service." >&2
    echo "Please configure your resolver to use 127.0.0.1 manually." >&2
elif grep -q "Generated by" /etc/resolv.conf; then
    echo "Note: /etc/resolv.conf is managed by another service. Manual configuration may be needed." >&2
    echo "To use wildcard DNS, add 'nameserver 127.0.0.1' as the first line in /etc/resolv.conf" >&2
else
